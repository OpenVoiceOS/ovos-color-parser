# -*- coding: utf-8 -*-
"""Azerbaijani color-term resolution against the bundled ``az`` wordlists."""
import unittest
...
class TestAzerbaijani(unittest.TestCase):
    def test_basic_colors_resolve(self):
        for word, check in (('qırmızı', _is_reddish),
                            ('mavi', _is_bluish),
                            ('yaşıl', _is_greenish)):
            with self.subTest(word=word):
                c = color_from_description(word, "az")
    ...
    def test_darkness_modifier_lowers_lightness(self):
        base = color_from_description('qırmızı', "az")
        dark = color_from_description('tünd qırmızı', "az")
        self.assertIsNotNone(dark)
        self.assertLess(dark.as_hls.l, base.as_hls.l)

    def test_gibberish_returns_none(self):
        self.assertIsNone(color_from_description("qzxwvqq", "az"))

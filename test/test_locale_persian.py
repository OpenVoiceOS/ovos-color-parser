# -*- coding: utf-8 -*-
"""Persian color-term resolution against the bundled ``fa`` wordlists."""
import unittest
...
class TestPersian(unittest.TestCase):
    def test_basic_colors_resolve(self):
        for word, check in (('قرمز', _is_reddish),
                            ('آبی', _is_bluish),
                            ('سبز', _is_greenish)):
            with self.subTest(word=word):
                c = color_from_description(word, "fa")
    ...
    def test_darkness_modifier_lowers_lightness(self):
        base = color_from_description('قرمز', "fa")
        dark = color_from_description('قرمز تیره', "fa")
        self.assertIsNotNone(dark)
        self.assertLess(dark.as_hls.l, base.as_hls.l)

    def test_gibberish_returns_none(self):
        self.assertIsNone(color_from_description("qzxwvqq", "fa"))

# -*- coding: utf-8 -*-
"""Turkish color-term resolution against the bundled ``tr`` wordlists."""
import unittest
...
class TestTurkish(unittest.TestCase):
    def test_basic_colors_resolve(self):
        for word, check in (('kırmızı', _is_reddish),
                            ('mavi', _is_bluish),
                            ('yeşil', _is_greenish)):
            with self.subTest(word=word):
                c = color_from_description(word, "tr")
    ...
    def test_darkness_modifier_lowers_lightness(self):
        base = color_from_description('kırmızı', "tr")
        dark = color_from_description('koyu kırmızı', "tr")
        self.assertIsNotNone(dark)
        self.assertLess(dark.as_hls.l, base.as_hls.l)

    def test_gibberish_returns_none(self):
        self.assertIsNone(color_from_description("qzxwvqq", "tr"))
